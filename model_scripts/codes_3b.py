...
cache_dir = './../models/codes-3b'

tokenizer = AutoTokenizer.from_pretrained('seeklhy/codes-3b', cache_dir=cache_dir)
#model = AutoModelForCausalLM.from_pretrained('seeklhy/codes-3b', torch_dtype=torch.float16, cache_dir=cache_dir)
model = AutoModelForCausalLM.from_pretrained('seeklhy/codes-3b', torch_dtype=torch.float32, cache_dir=cache_dir)
model = model.to("cpu")
model.eval()
...
def generate_sqlNEW(model, input_text, tokenizer, max_length=256):
